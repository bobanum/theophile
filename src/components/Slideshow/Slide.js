 ...
 * @extends {Webponent}
 */
export class Slide extends Webponent {
	connectedCallback() {
	...
	static async init(Theophile) {
		...
		this.split = this.split || "br,.split";
		this.exclude = this.exclude || "h1:not(.full)+h2, h2:not(.full)+h3, h1:not(.full)+h3, .skip";
	...
	dom = {
		...
		body() {
			...
			result.querySelectorAll("iframe:not([src^='http']):not([src^='data:']), object").forEach(element => {
		...
		footer: () => {
			...
			var copyright = result.appendChild(document.createElement("div"));
			copyright.innerHTML = this.footertext;
			result.appendChild(this.dom.slideNumber());
			return result;
		},
		slideNumber: () => {
			var slideNumber = document.createElement("div");
			slideNumber.classList.add("number");
	...
	applyStyles(container) {
		container.querySelectorAll("[data-style]").forEach(element => {
	...
	parseElementStyle(from, to) {
		...
		Slide.applyStyle(from.getAttribute("data-style"), to);
		from.removeAttribute("data-style");
		return this;
	}
	static showContactsheet() {
		document.documentElement.classList.add("th-contactsheet");
		var contactsheet = document.body.appendChild(this.dom.contactsheet());
	...
	static get html_status() {
		if (!this._html_status) {
			...
			result.appendChild(this.dom.clock());
			result.appendChild(this.dom.timeSlide());
			result.appendChild(this.dom.timeSlideshow());
			result.addEventListener("click", e => {
				var next = (e.target.nextSibling || e.currentTarget.firstChild);
				e.target.style.display = "none";
				next.style.display = "";
			});
			Array.from(result.children).forEach(element => element.style.display = "none");
			result.firstChild.style.display = "";
			this._html_status = result;
		}
	...
	static async process() {
		...
		document.documentElement.style.setProperty("--nlines", this.nlines);
		document.documentElement.style.setProperty("--ratio", Math.round(ratio * 1000) / 1000);
		var style = document.head.appendChild(document.createElement("style"));
		var innerHTML = "";
		innerHTML += `@media (min-aspect-ratio: ${Math.round(ratio * 1000)} / 1000) {.th-slideshow .th-slide {--font-size: calc(100vh / var(--nlines));}}`;
		innerHTML += `@media (min-aspect-ratio: ${Math.round(ratio * 1200)} / 1000) {.navigation > .previous, .navigation > .next {opacity:.5;background:none}}`;
		innerHTML += `@media (max-aspect-ratio: ${Math.round(ratio * 900)} / 1000) {.navigation > .options {opacity:1;background:none;}}`;
	...
	ajustZoom() {
		...
		var backdrop = document.body.appendChild(
			this.constructor.dom.backdrop()
		);
		backdrop.appendChild(this.html);
		var body = this.html.querySelector(".body");
	...
	static startSlideshow(state = true) {
		if (state) {
			...
			this.backdrop = document.body.appendChild(this.dom.backdrop());
